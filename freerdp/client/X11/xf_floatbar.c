...
void xf_floatbar_thread(LPVOID arg)
{
	xfWindow* window = (xfWindow*)arg;
	xfFloatbar* floatbar = window->floatbar;

	int x_root  = floatbar->x;
	int x_width = floatbar->width;
	while (1) {
		xf_floatbar_hide_and_show(floatbar);

		if (x_root != floatbar->x) {
			int xr = floatbar->x;
			int target = 2;
			for (int i = 0; i < target; i++) {
				...
				if (xr == floatbar->x) {
					if (i == target - 1) {
						XMoveWindow(floatbar->xfc->display, floatbar->handle, floatbar->x, floatbar->y);
						x_root = floatbar->x;
					}
				} else {
					break;
		...
		if (x_width != floatbar->width) {
			int xw = floatbar->width;
			int target = 2;
			for (int i = 0; i < target; i++) {
				USleep(1000);
				if (xw == floatbar->width) {
					if (i == target - 1) {
						XMoveResizeWindow(floatbar->xfc->display, floatbar->handle, floatbar->x, floatbar->y, floatbar->width, floatbar->height);
						x_width = floatbar->width;
					}
				} else {
					break;
				}
